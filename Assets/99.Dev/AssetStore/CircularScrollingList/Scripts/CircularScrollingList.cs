...
namespace AirFishLab.ScrollingList
{
    ...
    public class CircularScrollingList : MonoBehaviour, IBeginDragHandler, IDragHandler, IEndDragHandler, IScrollHandler
    {
        ...
        [SerializeField]
        [Tooltip("The game object that stores the contents for the list to display. " + "It should be derived from the class BaseListBank.")]
        private BaseListBank _listBank;
        [SerializeField]
        [Tooltip("The game objects that used for displaying the content. " + "They should be derived from the class ListBox")]
        ...
        {
            _listPositionCtrl = new ListPositionCtrl(_setting, _rectTransform, _canvasRefCamera, _listBoxes);
            _listContentManager = new ListContentManager(_setting, _listBank, _listBoxes.Count);
            ...
            { 
                _listBoxes[i].Initialize(_setting, _listPositionCtrl, _listContentManager, _listBoxes, i);
            }
        ...
        public void Refresh(int centeredContentID = -1)
        {
            ...
            { 
                centeredContentID = centeredBox.contentID == int.MinValue ? 0 : Mathf.Min(centeredBox.contentID, numOfContents - 1);
            }
            else if (centeredContentID >= numOfContents)
            {

                throw new IndexOutOfRangeException( $"{nameof(centeredContentID)} is larger than the number of contents");
            }
        ...
        #region Add parkjinbeom
        /// <summary>
        /// 패턴색을 변경시 스크롤 아이탬 및 패턴 아이템의 색을 변경
        /// </summary>
        [System.Obsolete]
        public void ChangeColor()
        {
            Color color = this.transform.GetChild(1).GetComponent<UnityEngine.UI.Image>().color;
           // HandPaintSceneManager.Instance.ChageColorPatternObject(color);
            //패턴색 변경후 퀘스트 검사
            //QuestHandPaintSceneManager.Instance.QuestConfirm();
        }

        #endregion Add parkjinbeom
    }
}
