...
public class BitmapBrushesTools : MonoBehaviour
{
	public static void DrawLineWithBrush(Vector2 start, Vector2 end, AdvancedMobilePaint paintEngine)
	{
		...
		int minDistance = (int)(paintEngine.brushSize >> 2); // divide by 2, you might want to set mindistance to smaller value, to avoid gaps between brushes when moving fast
		int pixelCount = 0;
		int e2;
		while (loop)
		{
	...
	public static void DrawCustomBrush2(int px, int py, AdvancedMobilePaint paintEngine)
	{
		...
		{
			randomRotateValue = UnityEngine.Random.Range(0, paintEngine.textureByteTest.Count);
			test = paintEngine.textureByteTest[randomRotateValue];
		}
							...
							if (/*(paintEngine.useLockArea && paintEngine.lockMaskPixels[pixel] == 1) ||*/ !paintEngine.useLockArea)
							{
								switch (paintEngine.brushMode)
								{
									...
									case BrushProperties.Default:
										...
										break;
									case BrushProperties.Simple:
										if (paintEngine.customBrushBytes[brushPixel + 3] > 100)
										{
				...
				while (Mathf.Abs(x0 - x1) > 1)
				{
					...
					if (x0 < paintEngine.texWidth && x0 >= 0 && y0 < paintEngine.texHeight && y0 >= 0)
					{
						...
						else
						{
							...
							/*int*/
							pixel2 = (int)Mathf.Repeat((paintEngine.customPatternWidth * xx + yy) * 4, /*patternBrushBytes*/patternSource.Length);
